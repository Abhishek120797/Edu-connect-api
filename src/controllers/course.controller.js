	...
	if (course_object) {
		...
		if (deleted) {
			return res
				.status(201)
				.json(
					new ApiResponse(201, deleted, "Course deleted successfully")
				);
		} else {
			throw new ApiError(
				500,
				"internal server error while deleting course"
			);
		}
	} else {
		return res
	...
	if (category_object) {
		...
		if (courses) {
			return res
				.status(201)
				.json(
					new ApiResponse(201, courses, "courses fetched succesfully")
				);
		} else {
			throw new ApiError(
				500,
